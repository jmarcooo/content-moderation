...
const ImageViewer = {
    ...
    reset() {
        this.scale = 1; this.rotate = 0; this.flip = 1;
        this.update();
    },
...
const ReviewApp = {
    ...
    clearAllHighlights() {
        const boxes = document.querySelectorAll('.text-context-box');
        boxes.forEach(box => {
            // Very basic clear: remove span tags
    ...
    handleTextSelection(slot) {
        ...
        if (selection.rangeCount > 0 && !selection.isCollapsed && box.contains(selection.anchorNode)) {
            this.clearAllHighlights(); // Enforce single selection
    ...
    // --- DRAWER & VIOLATIONS ---
    renderViolations() {
        if (!Config || !Config.violations) return;
    ...
    validateSubmission() {
        ...
        if (isVisible(contentModule)) {
            if (!contentModule.querySelector('.btn-sm.active')) {
        ...
        if (isVisible(textModule)) {
            ...
            if (activeBtn.textContent.trim() === 'Restrict') {
                if (textModule.querySelectorAll('.restricted-text').length === 0) {
