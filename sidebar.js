// ===========================================
// 1. SIDEBAR HTML GENERATION
// ===========================================
const APP_LANG = localStorage.getItem('appLang') || 'en';

// (Translations kept brief for this snippet, same as before)
const TRANSLATIONS = {
    en: { brand: "AdminPanel", overview: "Overview", users: "User Management", mod: "Moderation Queue", audit: "Quality Audit", appeal: "Appeal Center", notifs: "Notifications", settings: "Settings", logout: "Logout", online: "Online", break: "Break", lunch: "Lunch", meeting: "Meeting", idle: "Idle", training: "Training", offline: "Offline" },
    zh: { brand: "管理后台", overview: "总览", users: "用户管理", mod: "审核队列", audit: "质量质检", appeal: "申诉中心", notifs: "通知中心", settings: "设置", logout: "退出登录", online: "在线", break: "小休", lunch: "午餐", meeting: "会议", idle: "空闲", training: "培训", offline: "离线" }
};
const T = TRANSLATIONS[APP_LANG];

// SVG Icons (Reused)
const ICONS = {
    brand: `<svg class="brand-icon" viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zm0 9l2.5-1.25L12 8.5l-2.5 1.25L12 11zm0 2.5l-5-2.5-5 2.5L12 22l10-8.5-5-2.5-5 2.5z"/></svg>`,
    overview: `<svg class="nav-icon" viewBox="0 0 24 24"><path d="M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z"/></svg>`,
    users: `<svg class="nav-icon" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>`,
    mod: `<svg class="nav-icon" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>`,
    audit: `<svg class="nav-icon" viewBox="0 0 24 24"><path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>`,
    appeal: `<svg class="nav-icon" viewBox="0 0 24 24"><path d="M1 21h12v2H1zM5.245 8.07l2.83-2.827 14.14 14.142-2.828 2.828zM12.317 1l5.657 5.656-2.83 2.83-5.654-5.66zM3.825 9.485l5.657 5.657-2.828 2.828-5.657-5.657z"/></svg>`,
    notifs: `<svg class="nav-icon" viewBox="0 0 24 24"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.63-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.64 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2zm-2 1H8v-6c0-2.48 1.51-4.5 4-4.5s4 2.02 4 4.5v6z"/></svg>`,
    ...
    logout: `<svg class="nav-icon" style="fill:#f85149;" viewBox="0 0 24 24"><path d="M10.09 15.59L11.5 17l5-5-5-5-1.41 1.41L12.67 11H3v2h9.67l-2.58 2.59zM19 3H5c-1.11 0-2 .9-2 2v4h2V5h14v14H5v-4H3v4c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z"/></svg>`,
...
<nav class="sidebar">
    ...
    <ul class="nav-links" style="margin-top: auto; border-top: 1px solid var(--border-color); padding-top: 10px;">
        <li>
...
// (Rest of the JS logic remains exactly the same as previous steps)
// ... [Auto-Translate, Theme Logic, Status Logic] ...
// Copy the rest of the logic from the previous sidebar.js update
// ===========================================
// 4. AUTO-TRANSLATE & LOGIC (Preserved)
// ===========================================
...
function updateThemeIcon(mode) { themeBtn.innerHTML = (mode === 'dark') ? ICONS.moon : ICONS.sun; }
if (savedTheme === 'dark') { document.body.classList.add('dark-mode'); updateThemeIcon('dark'); } 
...
window.toggleStatusMenu = function() { document.getElementById('status-menu').classList.toggle('active'); }
document.addEventListener('click', function(event) {
    const dropdown = document.querySelector('.status-dropdown');
    const menu = document.getElementById('status-menu');
    if (dropdown && !dropdown.contains(event.target)) menu.classList.remove('active');
});
